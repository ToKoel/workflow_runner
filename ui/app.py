...
class WorkflowBox(Widget):
    BINDINGS = [Binding("enter", "run_workflow", "Run", priority=True)]
    ...
    def on_list_view_highlighted(self, event: ListView.Highlighted) -> None:
        self.app.current_tab_idx = 0
        self.app.update_tab_ui()
        if event.item and event.item.id:
            self.post_message(self.Selected(event.item.id))

    def on_list_view_selected(self, event: ListView.Selected) -> None:
        self.post_message(self.Run(event.item.id))


class SettingsBox(Widget):
...
#save_settings:focus {
    ...
    def on_workflow_box_selected(self, event: WorkflowBox.Selected) -> None:
        ...
        self.query_one(
            "#detail-body", Static).update(formatted_details)

    def on_workflow_box_run(self, event: WorkflowBox.Run) -> None:
        if event.workflow_id and not self.is_running:
            self.is_running = True
            self.continuous_timer_loop(time.time())
            self.run_workflow_async(event.workflow_id)
